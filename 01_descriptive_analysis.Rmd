...
# Distance Group about communities
...
communities_distances_hf <- communities_distances_hf %>% 
  ...
  pivot_longer(
    ...
    values_to = "distance_to_hf"
  ) %>% 
  mutate(distance_to_hf = as.numeric(distance_to_hf))

communities_distances_hf_cat <- comunidades_info_total %>% 
  filter(`Health Facility Name` %in% hf_shp$ffi_h_health_facility_name) %>% 
  select(
...
communities_distances_hf_cat <- communities_distances_hf_cat %>% 
  filter(ffi_is_health_facility_name != "SOLEDAD DE VILLA BELEN") %>% 
  group_by(ffi_is_health_facility_name) %>% 
  mutate(
    distance_to_hf = as.numeric(cut_number(distance_to_hf, 4)),
    distance_to_hf = factor(distance_to_hf, 
...
communities_distances_rh <- comunidades_info_total %>% 
  ...
  mutate(
    ...
    distance_rh_bydistr = as.numeric(value),
...
communities_distances_rh_cat <- communities_distances_rh %>% 
  drop_na(distance_rh_bydistr) %>% 
  arrange(ffi_is_district, distance_rh_bydistr) %>% 
  group_by(ffi_is_district) %>% 
  mutate(
    distance_rh_bydistr = as.numeric(cut_number(distance_rh_bydistr, 4)),
    distance_rh_bydistr = factor(distance_rh_bydistr, 
                            labels = c("Proximate", "Moderate",
                                       "Distant", "Extra Distant"))
  )
```
...
```{r}
sero_timeofmalaria_district_distance_hf <- seropositivy_summarise(
    ffi_total,
    "timeofmalaria",
    age_cat,
    distance_to_hf
  ...
  ungroup() %>% 
  mutate(
    distance_to_hf = fct_relevel(distance_to_hf, 
                                 "Proximate", "Moderate",
                                 "Distant", "Extra Distant")
  ) %>% 
  ggplot(
  ...
  labs(
    x = "Age",
    y = "Seropositivity",
    title = str_wrap("Seropositivity according to Time of Exposure to malaria by District and distance category of the communities to their respective Health Facilities", 80)
  ) +
  guides(
    color = guide_legend("Malaria")
  ) +
...
sero_timeofmalaria_district_distance_hf
```

```{r eval=FALSE}
ggsave(
  "./02_output/plots/plot36_sero_timeofmalaria_district_distance_hf.png",
  sero_timeofmalaria_district_distance_hf,
  dpi = 300,
  bg = "white",
  width = 12,
...
```{r}
sero_timeofmalaria_district_distance_rh <- seropositivy_summarise(
    ffi_total,
    "timeofmalaria",
    age_cat,
    distance_rh_bydistr
  ...
  ungroup() %>% 
  mutate(
    distance_rh_bydistr = fct_relevel(distance_rh_bydistr, 
                                      "Proximate", "Moderate",
                                      "Distant", "Extra Distant")
  ) %>% 
  ggplot(
  ...
  labs(
    x = "Age",
    y = "Seropositivity",
    title = str_wrap("Seropositivity according to Time of Exposure to malaria by District and distance category of the communities to Iquitos Regional Hospital", 80)
  ) +
  guides(
    color = guide_legend("Malaria")
  ) +
...
sero_timeofmalaria_district_distance_rh
```

```{r eval=FALSE}
ggsave(
  "./02_output/plots/plot37_sero_timeofmalaria_district_distance_rh.png",
  sero_timeofmalaria_district_distance_rh,
  dpi = 300,
  bg = "white",
  width = 12,
