...
communities_distances_hf_sampling <- communities_distances_hf_sampling %>% 
  ...
  pivot_longer(
    ...
    values_to = "distance_to_hf_sampl"
  ) %>% 
  mutate(distance_to_hf_sampl = as.numeric(distance_to_hf_sampl))
...
communities_distances_hf_sampling_cat <- communities_distances_hf_sampling_cat %>% 
  ...
  mutate(
    distance_to_hf_sampl = as.numeric(cut_number(distance_to_hf_sampl, 4)),
...
communities_distances_rh_sampling <- enframe(communities_distances_rh_sampling) %>%
  mutate(
    ...
    ffi_is_community = communities_sf$ffi_h_community,
    distance_to_rh_sampl = as.numeric(value),
    ffi_is_district = communities_sf$ffi_h_district
  ) %>%
  select(-c(name, value)) %>% 
  group_by(ffi_is_district) %>% 
  mutate(
    distance_to_rh_sampl = as.numeric(cut_number(distance_to_rh_sampl, 4)),
...
communities_distances_hf_cat <- communities_distances_hf_total %>% 
  ...
  mutate(
    ...
    distance_to_hf_census = factor(distance_to_hf_census, 
                            labels = c("Proximate", "Moderate",
                                       "Distant", "Extra Distant"))
  ) %>% 
  bind_rows(
...
communities_distances_rh_cat <- communities_distances_rh %>% 
  ...
  mutate(
    ...
    distance_to_rh_census = factor(distance_to_rh_census, 
                            labels = c("Proximate", "Moderate",
                                       "Distant", "Extra Distant"))
  ) %>% 
  distinct(ffi_is_community, ffi_is_district)
```
...
```{r}
ggsave("02_output/plots/map_communities_hf_dashed.png",
       map_linedash,
       dpi = 400,
...
sero_timeofmalaria_district_distance_rh_census <- seropositivy_summarise(
  ...
  labs(
    x = "Age",
    y = "Seropositivity",
    title = str_wrap("Seropositivity according to Time of Exposure to malaria by District and distance category of the communities to Iquitos Regional Hospital on a Population Basis", 80)
  ) +
  guides(
    color = guide_legend("Malaria")
  ) +
...
sero_timeofmalaria_district_distance_rh_sampling <- seropositivy_summarise(
  ...
  labs(
    x = "Age",
    y = "Seropositivity",
    title = str_wrap("Seropositivity according to Time of Exposure to malaria by District and distance category of the communities to Iquitos Regional Hospital on a Sampling Basis", 80)
  ) +
  guides(
    color = guide_legend("Malaria")
  ) +
